...
#   Stop-ScheduledTask -TaskName Bentlyk
...
if (-not (Test-Path (Join-Path $repo ".env"))) {
    Write-Warning "No .env in $repo - create it (keys + BENTLYK_ALLOW_CODE=1) first."
}

$action = New-ScheduledTaskAction -Execute $pythonw -Argument "-m bentlyk worker --interval 120" -WorkingDirectory $repo
$trigger = New-ScheduledTaskTrigger -AtLogOn
$settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -StartWhenAvailable -Hidden -RestartCount 999 -RestartInterval (New-TimeSpan -Minutes 1) -ExecutionTimeLimit ([TimeSpan]::Zero)

Register-ScheduledTask -TaskName "Bentlyk" -Action $action -Trigger $trigger -Settings $settings -Description "Bentlyk autonomous resident" -Force | Out-Null

Start-ScheduledTask -TaskName "Bentlyk"
Write-Host "Bentlyk is now a resident: background service, autostarts at logon, restarts on failure."
Write-Host "Watch it on the dashboard. Stop with: Stop-ScheduledTask -TaskName Bentlyk"
